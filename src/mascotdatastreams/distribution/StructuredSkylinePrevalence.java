...
public class StructuredSkylinePrevalence extends Dynamics implements Loggable {	
		...
		for (int j = 0; j < NeInt.length; j++){
			if (parametricFunction.get(j).isTime) {
				if (parametricFunction.get(j) instanceof SplinePrevalenceToNe) {
					NeInt[j] = ((SplinePrevalenceToNe) parametricFunction.get(j)).getPrevalenceTime(t);
				}else {
