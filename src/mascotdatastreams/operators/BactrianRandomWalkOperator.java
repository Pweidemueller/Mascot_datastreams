...
public class BactrianRandomWalkOperator extends KernelOperator {
    ...
    public final Input<Integer> indexInput = new Input<>("index",
            "0-based index of the entry of the parameter to operate on (no random choice). If not set, defaults to 0.",
            0);
    ...
    public double proposal() {
        ...
        int i = indexInput.get();
        if (i < 0 || i >= param.getDimension()) {
            throw new IllegalArgumentException("Index " + i + " is out of range for parameter of dimension " + param.getDimension());
        }
