...
/**
 * Vendored fork of mascot.ode.Euler2ndOrder (Mascot v3.0.7) with one fix:
 * initWithIndicators(...) now allocates sumDotStates, matching the behaviour
 * of init(...) and initAndcalculateValues(int, ...). Without that allocation
 * computeSecondDerivate hits a NullPointerException on the first ODE step.
 *
 * MascotLogPflag.doEulerAtTime uses initWithIndicators to enable the
 * indicator-based sparsity optimisation in the inner loops; that path is the
 * reason we need this fork. The old (grid-based) MascotLogPflag.doEuler path
 * goes through initAndcalculateValues(int, ...) which is unaffected.
 *
 * Drop this fork once an upstream Mascot release ships the fix and is
 * referenced via deps/Mascot.
 */
public class Euler2ndOrder implements Euler2ndOrderBase {
	...
	public void initWithIndicators(double[] migration_rates, int[] indicators, double[] coalescent_rates, int lineages) {
        ...
        this.lineages = lineages;
        this.dimension = this.lineages*this.states;
    	// vendored-fork fix: upstream forgets the sumDotStates allocation here,
    	// which makes computeSecondDerivate NPE on the first ODE step. The
    	// sibling init(...) and initAndcalculateValues(int, ...) overloads both
    	// allocate it; this one didn't. Once a fixed Mascot release is
    	// available upstream, drop this fork and call the upstream class.
    	sumStates = new double[states];tCR = new double[states]; sumDotStates = new double[states];
    	hasIndicators = true;
    	hasMultiplicator = false;
